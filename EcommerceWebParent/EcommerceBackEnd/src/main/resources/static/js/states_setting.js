...
function loadStates4Country(){
	...
	$.get(url, function(responseJSON){
		...
		changeFormStateToNew();
		showToastMessage("All states have been loadded for country " + selectedCountry.text());
	}).fail(function(){
		showToastMessage("ERROR: Could not connect to server on server encountered an error");
	});
}
...
function deleteState(){
	...
	$.ajax({
		...
		changeFormStateToNew();
		showToastMessage("The states have been deleted");
	}).fail(function(){
		showToastMessage("ERROR: Could not connect to server or server encoutered an error");
	});
}
